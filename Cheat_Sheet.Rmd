...
```
Gender has been automatically dummified into 'GenderMale'
This model trains a different y-intercept for each gender, not slope
**ascess**
```{r}
...
**The Best model maximizes likelyhood**
+ The parameter estimates $(\beta_0, \beta_1, ...etc)$ that yield the highest likelyhood are called MLE (maximum likelyhood estimates)  
...
$$
\hat{y_i} = \begin{cases}
  Success(1) & \hat{p_i} \ge c\\ 
  Failure(0) & \hat{p_i} \le c
\end{cases}
...
**Comparing models**
Say we have a full logistic regression model with p predictors and a reduces model with a subset of p to compare: (sometimes called Residual Deviance)  
...
- Admit: A binary variable indicating whether or not a student was admitted to the graduate school.  
...
to see if a variable (in this case rank) changes the residuals, drop it  
```{r}
